...
data class PresetMode(
    ...
    companion object {
        ...
        fun getPresetModes(context: Context, identifier: String): ArrayList<PresetMode>? {
        ...
        private fun parseCSV(lines: List<String>): ArrayList<PresetMode> {
            ...
            for (line in lines) {
                ...
                val mode = parameters[CSV_MODE].toInt()
                ...
                val bits =
                    if (parameters[CSV_BITS] == sNull) null else parameters[CSV_BITS].split(",")
                        .map { it.toInt() }
                val colorFPS =
